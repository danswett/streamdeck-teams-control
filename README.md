...
The bundled profile is laid out for exactly this: one page that serves both
roles, so taking control mid-session re-lights the deck rather than stranding
you on a layout that no longer applies.

#### Switching profile automatically

The plugin ships a **PowerPoint Live** profile and can switch to it the moment a
deck starts being presented, then switch back when it stops.
...
Stream Decks, and only ever switches back the devices it switched away, so a
profile you pick by hand mid-presentation is left alone.

Stream Deck only lets a plugin switch to profiles it ships itself, so the layout
is generated by `tools/build-profile.ts` rather than made by hand.
